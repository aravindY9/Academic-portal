...
if (isset($_GET['id'])) {
    ...
    $sql = "DELETE FROM exams WHERE ExamID = '$id'"; // Assuming 'CODE' is the column for course codes

    if (mysqli_query($connection, $sql)) {
        $response=array();

        echo json_encode($response["message"]= "exam with code $id has been deleted.");
    } else {
        $response=array();
