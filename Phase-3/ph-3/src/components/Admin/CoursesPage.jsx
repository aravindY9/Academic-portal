...
function CoursePage(){
    ...
    useEffect(() => {
        fetch("http://localhost/A/admin/coursedata.php", {
            credentials: 'include',
          })
        .then((response) => {
            if (response.ok) {
    ...
    const deleteUser = (id) => {
        fetch(`http://localhost/A/admin/deletecourse.php?id=${id}`)
    ...
    return <TableEntries name={data.COURSE} id={data.CODE} info={data.INSTRUCTORID} edit={`/admin/editcourse/${data.CODE}`} deleteAccount={() => deleteUser(data.CODE)}/>
